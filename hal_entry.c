...
void hal_entry(void)
{
    ...
    while (1)
    {
        ...
        {
            if(time_pendulum) Fuz_pendulum();
            if(time_cart) Fuz_cart();
...
void interrupt_pos_irc(external_irq_callback_args_t *p_args)
{
    ...
    if(state) e_pos+=0.5f;       //rotate left => +++
    else e_pos-=0.5f;            //rotate right => --
}
...
void interrupt_pen_irc(external_irq_callback_args_t *p_args)
{
    ...
    if(!state) e_angle+=0.5f;    //rotate right => ++
    else e_angle-=0.5f;          //rotate left => --
}
